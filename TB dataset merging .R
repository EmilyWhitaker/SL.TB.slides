...
TB.zoops= read.csv('data/TBzoops_Clean.csv', stringsAsFactors = F)
TB.zoops$sampledate = ymd(TB.zoops$sampledate)
TB.zoops$year = year(TB.zoops$sampledate)

CleanTB.zoops.subset = subset(TB.zoops, year >= 2008)



TB.Phytos= read.csv('data/TBPhytos_Clean.csv', stringsAsFactors = F)
TB.Phytos$sampledate = ymd(TB.Phytos$sampledate)

TB_Clean= read.csv('data/TB_IceBD.clean.csv', stringsAsFactors = F)
TB_Clean$sampledate = mdy(TB_Clean$sampledate)


TB.Phytos.clean = TB.Phytos %>% select(TBphytoDate = sampledate, division, genus, taxa_name, relative_total_biovolume, cells_per_ml,biovolume_conc) %>%
  mutate(datePlus1 = TBphytoDate + 1) %>% mutate(dateMinus1 = TBphytoDate - 1)

DataPhytos.clean  <- fuzzy_left_join(TB_Clean, TB.Phytos.clean, by = c("sampledate" = "datePlus1", "sampledate" = "dateMinus1"),
                                    match_fun = list(`<=`, `>=`))
write.csv(DataPhytos.clean, 'data/TBminusZoops.csv', row.names = F)
######make it TB
CleanTBZoops = CleanTB.zoops.subset %>% select(TBZoopDate = sampledate, species_code, species_name, density, individuals_measured, avg_length) %>%
  mutate(datePlus1 = TBZoopDate + 1) %>% mutate(dateMinus1 = TBZoopDate - 1)

TBdataset.clean  <- fuzzy_left_join(DataPhytos.clean, CleanTBZoops, by = c("sampledate" = "datePlus1", "sampledate" = "dateMinus1"),
                                    match_fun = list(`<=`, `>=`))

write.csv(TBdataset.clean, 'data/TBCleanSet07212020.csv', row.names = F)
...
write.csv(SPdataset.clean, 'data/SPCleanSet07212020.csv', row.names = F)
